...
const NAVIGATION: Record<UserRole, NavItem[]> = {
  ...
  DOCENTE: [
    { label: 'Dashboard', route: '/dashboard', icon: 'grid' }, { label: 'Proyectos / Revisiones', route: '/proyectos', icon: 'grid' }, { label: 'Producción', route: '/produccion', icon: 'grid' }, { label: 'Portal académico', route: '/portal', icon: 'grid' }, { label: 'Reportes', route: '/reportes', icon: 'grid' }, { label: 'Configuración', route: '/configuracion', icon: 'settings' },
  ],
  ESTUDIANTE: [
...
export class MainLayoutComponent {
  readonly authService = inject(AuthService); private readonly router = inject(Router); readonly menuOpen = signal(false); readonly user = computed(() => this.authService.user); readonly navItems = computed(() => NAVIGATION[this.user()?.rol ?? 'ESTUDIANTE']);
