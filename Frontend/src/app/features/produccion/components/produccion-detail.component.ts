...
export class ProduccionDetailComponent implements OnInit, OnDestroy {
 private readonly dialogos=inject(ConfirmacionService); private readonly route=inject(ActivatedRoute); private readonly router=inject(Router); private readonly auth=inject(AuthService); private readonly service=inject(ProduccionService); private readonly http=inject(HttpClient);
 id=''; orden?:OrdenProduccion; etapas:OrdenProduccionEtapa[]=[]; etapa?:OrdenProduccionEtapa; evidencias:EvidenciaVista[]=[]; revisiones:RevisionEtapa[]=[]; observacion=''; descripcion=''; archivo?:File; preview?:string; mensaje=''; cargando=true; guardando=false; checks=[false,false,false]; modal:''|'iniciar'|'completar'=''; visor?:string; resultadoRevision:"APROBADA"|"OBSERVADA"="APROBADA"; observacionRevision="";
 ngOnInit():void { this.route.paramMap.subscribe(p=>{this.id=p.get('id')??'';this.cargar();}); }
 ngOnDestroy():void { this.limpiarPreview(); }
 get estudiante():boolean{return this.auth.user?.rol==='ESTUDIANTE';} get docente():boolean{return this.auth.user?.rol==='DOCENTE';} get enProceso():boolean{return this.estudiante && ['EN_PROCESO','OBSERVADA'].includes(this.etapa?.estado??'') && this.orden?.estado==='EN_PROCESO';} get completada():number{return this.etapas.filter(e=>e.estado==='APROBADA').length;} get progreso():number{return this.etapas.length?Math.round(this.completada*100/this.etapas.length):0;} get checklistListo():boolean{return this.checks.every(Boolean)&&this.evidencias.length>0;} get puedeRevisar():boolean{return this.docente && this.etapa?.estado==="EN_REVISION";} get ultimaRevision():RevisionEtapa|undefined{return this.revisiones[this.revisiones.length-1];}
 cargar():void {if(!this.id)return;this.cargando=true;this.service.obtenerOrden(this.id).subscribe({next:o=>{this.orden=o;this.service.etapas(this.id).subscribe({next:e=>{this.etapas=e;this.seleccionar(e.find(x=>x.estado==='EN_PROCESO'||x.estado==='OBSERVADA'||x.estado==='EN_REVISION')??e[0]);this.cargando=false;},error:x=>this.fallo(x)});},error:x=>this.fallo(x)});}
 ...
 eliminar(e:EvidenciaVista):void {if(!this.etapa||!this.enProceso)return;void this.dialogos.solicitar('Eliminar evidencia','La evidencia se eliminará de esta etapa.','Eliminar').then(ok=>{if(ok&&this.etapa)this.guardar(this.service.eliminarEvidencia(this.id,this.etapa.id,e.id),'Evidencia eliminada.',()=>this.seleccionar(this.etapa!));});}
 completar():void {if(!this.etapa||!this.checklistListo)return;this.guardar(this.service.completarEtapa(this.id,this.etapa.id),'Etapa enviada a revisión docente.',()=>{this.modal='';this.cargar();});}
 registrarRevision(resultado:"APROBADA"|"OBSERVADA"):void {if(!this.etapa||!this.puedeRevisar)return;const observacion=this.observacionRevision.trim();if(resultado==="OBSERVADA"&&!observacion){this.mensaje="La observación es obligatoria cuando el resultado es OBSERVADA.";return;}this.guardar(this.service.crearRevision(this.id,this.etapa.id,{resultado,observacion:observacion||undefined}),"Revisión registrada correctamente.",()=>{this.observacionRevision="";this.cargar();});}
 ...
 abrirVisor(url:string):void{this.visor=url;} cerrarVisor():void{this.visor=undefined;} private limpiarPreview():void{if(this.preview)URL.revokeObjectURL(this.preview);this.preview=undefined;}
}
