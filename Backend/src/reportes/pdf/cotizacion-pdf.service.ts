...
export class CotizacionPdfService {
  async generar(data: CotizacionPdfData): Promise<Buffer> {
    ...
    const numero=(value:string|number,max=4)=>new Intl.NumberFormat('en-US',{minimumFractionDigits:0,maximumFractionDigits:max,useGrouping:false}).format(Number(value));const money = (value: string) => `$${numero(value,2)}`;
    ...
    [['Subtotal telas', money(data.resumen.subtotalTelas)], ['Subtotal insumos', money(data.resumen.subtotalInsumos)], ['Subtotal materiales', money(data.resumen.subtotalMateriales)], [`Mano de obra (${numero(data.resumen.porcentajeManoObra,2)}%)`, money(data.resumen.valorManoObra)], [`Ganancia (${numero(data.resumen.porcentajeGanancia,2)}%)`, money(data.resumen.valorGanancia)]].forEach(([label, value]) => line(`${label}:`, value));
